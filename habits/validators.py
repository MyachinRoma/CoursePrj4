...
class HabitValidator:
    def validate_time_needed(self, attrs):
        """Validates that time needed
        to perform a habit is less
        than 2 mins (120 secs)."""
    ...
    def validate_pleasant_habit(self, attrs):
        ...
        habit doesn't additionally
        have reward, related habit,
        ...
        if attrs.get("is_pleasant") and any(
            ...
            raise serializers.ValidationError(
                ...
                "reward,it can't have a "
                "related habit or reward "
        ...
        if not attrs.get("is_pleasant") and not any(
            ...
            raise serializers.ValidationError(
                ...
                "regularly and on specific time."
            )

    def validate_end_time(self, attrs):
        """Validates that a habit that
        should be performed several
        times per day has end time
        and this time is correct."""
        if (
                attrs.get("frequency")
                and "x" in attrs.get("frequency")
                and not attrs.get("end_time")
        ):
            raise serializers.ValidationError(
        ...
        if (
                attrs.get("frequency")
                and "x" not in attrs.get("frequency")
                and attrs.get("end_time")
        ):
            raise serializers.ValidationError(
                "End time should be only "
                "selected for habits "
        ...
        if (
                attrs.get("end_time")
                and attrs.get("time")
                and attrs.get("end_time") <= attrs.get("time")
        ):
            raise serializers.ValidationError(
    ...
    def validate_days_of_week(self, attrs):
        """Validates selection of specific
        days of week when a habit should be performed."""
        if (
                attrs.get("frequency")
                and "d" in attrs.get("frequency")
                and not attrs.get("days_of_week")
        ):
            raise serializers.ValidationError(
                "For a habit that should be"
                "performed on specific days"
                "of week such days should be selected."
            )
        if (
                attrs.get("frequency")
                and "d" not in attrs.get("frequency")
